class ContextMenu {
  ...
  createItemMarkup(data) {
    ...
    if (data.children && data.children.length > 0) {
      ...
      subMenu.classList.add("contextSubMenu");

      data.children.forEach((child) => {
        subMenu.appendChild(this.createItemMarkup(child));
      });
  ...
  init() {
    ...
    document.addEventListener("contextmenu", (e) => {
      ...
      if (contextMenu.contains(e.target)) {
        return; // don't reopen if right-clicked inside the open menu
      }
    ...
    this.targetNode.forEach((target) => {
      target.addEventListener("contextmenu", (e) => {
        ...
        document.querySelectorAll(".contextMenu").forEach((m) => m.remove()); // closeMenu - all prev opened
...
const arrowLeft = `<svg viewBox="0 0 12.7 12.7"><path d="M3.464 0c-.144 0-.288.052-.398.162a.56.56 0 0 0 .009.799l5.389 5.389-5.389 5.389a.565.565 0 0 0 .8.79l5.759-5.759a.56.56 0 0 0 0-.798L3.866.171A.56.56 0 0 0 3.464 0Z" fill="currentColor" stroke="currentColor" stroke-width="1"/></svg>`;

const menuItems = [
  { content: `Refresh` },
  {
    content: `More Options ${arrowLeft}`,
    children: [
      { content: "Option A" },
      { content: "Option B" }
    ]
